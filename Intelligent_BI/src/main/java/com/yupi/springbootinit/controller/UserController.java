...
public class UserController {
     ...
     */
    @PostMapping("/login")
    public String userLogin(UserLoginRequest userLoginRequest, HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (user == null) {
            ...
            redirectAttributes.addFlashAttribute("message","用户名不存在或者密码错误");
            return "redirect:/";
        }
        // 3. 记录用户的登录态
        request.getSession().setAttribute(USER_LOGIN_STATE, user);

        return "redirect:/bi/index";
    }
